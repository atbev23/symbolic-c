...
const void* queue_dequeue(queue_t* queue) {
    if (queue_is_empty(queue)) return NULL;            // don't dequeue from empty queue. should be obv but there are some real sickos out there that have never felt the touch of a stack underflow
